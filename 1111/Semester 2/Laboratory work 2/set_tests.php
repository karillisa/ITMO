...
$user = getUser($id);
$result = $pdo->query("SELECT user_tests.id AS user_tests_id, tests.title, tests.id  FROM user_tests INNER JOIN tests ON user_tests.test_id=tests.id WHERE user_tests.user_id = '{$user['id']}'");
$user_test_list = $result->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
...
<div id="wrapper">
  ...
  <div id="content-wrapper" class="d-flex flex-column">
    ...
    <div id="content">
      ...
      <form action="/scripts/add_user_test.php" method="post">
      ...
      <div id="pvk_wrapper" class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          <h1 class="h3 mb-0 text-gray-800">Назначение тестов</h1>
        </div>
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          <h2 class="h3 mb-0 text-gray-800">Пользователь: <?= $user['name'] ?></h2>
        </div>
        ...
        <?php foreach($user_test_list as $user_test): ?>
        <div class="row mb-6 mt-5 pvk_item">
          <div class="col-auto mr-3 mt-1" style="min-width: 300px">
            <h5 class="m-0 font-weight-bold text-primary pvk_name"><?= $user_test['title'] ?></h5>
          </div>
          <input class="pvk_id" type="hidden" name="test_id[]" value="<?= $user_test['id'] ?>">
          <a href="/scripts/remove_user_test.php?id=<?= $user_test['user_tests_id'] ?>" class="btn btn-danger btn-icon-split">
...
<script>
  $('#add_pvk').click(function(){
    ...
    tmp.find(".test_id").val($("#select_pvk option:selected" ).val());
