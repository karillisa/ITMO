...
$all_tests = getAllTestsResults($category_id);

$sum = 0;
$count = 0;
foreach ($tests as $test){
    $sum += getAVGForTestsArray($test['results']);
    $count ++;
}
if($count == 0)
    $avgUser = 0;
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                            ...
                            <div class="collapse show" id="collapseCardExample<?= $test['id'] ?>">
                                <div class="card-body">
                                    ...
                                    <h4 class="small font-weight-bold mt-3"><?= $result['date'] ?> (сложность: <?= $result['complexity'] ?>)<span
                                        ...
                                        <div class="chart_div d-inline-block align-top"
                                    ...
                                    <h5 class="font-weight-bold mt-4">Динамика: <?= (($dynamic>=0)?"+":"-").abs($dynamic) ?> <?= $test['measure'] ?> (изменение показателя: <?= abs($dynamic_percent) ?>%)</h5>
                    ...
                    <div class="row">
                        <div class="d-sm-flex align-items-center justify-content-between mb-4">
                            <h1 class="h3 mb-0 text-gray-800">Результат для группы тестов: </h1>
                            <h2 class="h3 mb-0 text-primary">
                                <?php
                                    if($avgAll > $avgUser){
                                        echo "Ниже среднего";
                                    } else if($avgAll == $avgUser){
                                        echo "Средний";
                                    } else {
                                        echo "Выше среднего";
                                    }
