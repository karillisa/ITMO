...
if (isset($_GET['id'])) {
    ...
    $tests = getTestsForUser($_GET['id'],0);
    $criteria = getCriteriaForUser($_GET['id']);
    $pvks = getPvkForUserByCriteria($_GET['id'], $criteria);
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 class="h3 mb-0 text-gray-800">Результаты тестирования
                        пользователя <?= $user["name"] . " " . $user['second_name'] ?></h1>
                </div>
                    ...
                    <div class="col-lg-10 col-md-10 col-sm-12">
                        ...
                        <?php foreach ($tests as $test): ?>
                            <div class="card shadow mb-3">
                                ...
                                <a href="#collapseCardExample<?= $test['id'] ?>" class="d-block card-header py-3"
                                ...
                                <div class="collapse show" id="collapseCardExample<?= $test['id'] ?>">
                                    <div class="card-body">
                                        ...
                                        foreach ($test['results'] as $result):
                                            ...
                                            <div class="chart_div align-top"
                                                 data-start="<?= $result['start_pulse'] ?>"
                                                 data-end="<?= $result['end_pulse'] ?>"
                                                 data-during='<?= $result['during_pulse'] ?>'></div>
                                            <a class="btn btn-warning d-inline-block  align-top mb-2" href="/edit_test_pulse.php?result_id=<?= $result['id'] ?>">
                                                Редактировать пульс
                                            </a>
                                            <div class="progress mb-4">
                                                <div class="progress-bar bg-success" role="progressbar"
                                                     style="width: <?= $result['result'] / $max * 100 ?>%"
                                                     aria-valuenow="20" aria-valuemin="0" aria-valuemax="100"></div>
                                            </div>
                                        <?php endforeach; ?>

                                        <h5 class="font-weight-bold mt-4">
                                            Динамика: <?= (($dynamic >= 0) ? "+" : "-") . abs($dynamic) ?> мс
                                            (<?= (($dynamic >= 0) ? "ухудшилась" : "улучшилась") ?>
                                            на <?= abs($dynamic_percent) ?>%)</h5>
...
<script type="text/javascript" src="/js/draw_charts.js">
</script>
</body>
